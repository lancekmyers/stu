  ...
  where
    ...
    kwArgs =
      Builder.intercalate
        ","
        [Builder.text kw <> "=" <> prettyExp v | (kw, v) <- kws]
prettyExp (PyMethod x f xs) =
  (prettyExp x) <> "."
    <> (Builder.text f)
    <> "("
    <> Builder.intercalate "," (prettyExp <$> xs)
    <> ")"
